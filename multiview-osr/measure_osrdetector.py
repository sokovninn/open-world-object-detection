...
def main(opt, model):
    ...
    # classes_mean = torch.diag(torch.Tensor([10 for i in range(20)])).cpu()	
    ...
    print(in_emb.shape, classes_mean.shape)
    ...
    # in_score = (1 - softmin(in_dists)) * in_dists
    # print(softmin(in_dists).shape)
    # ood_score = (1 - softmin(out_dists)) * out_dists
    # print(in_score.shape, ood_score.shape)

    # print(torch.min(in_score, 1)[0].shape)
    # auroc = get_roc_sklearn(torch.min(in_score, 1)[0].cpu().tolist(),torch.min(ood_score, 1)[0].cpu().tolist())
...
def run_ood_distance(opt):
    ...
    for dir in os.listdir(experiments_dir):
        ...
        print(exp_name, dataset, int(num_classes[2:]))
