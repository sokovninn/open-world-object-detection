...
class Tiny_ImageNet_Filter(datasets.ImageFolder):
    ...
    """
    def __Filter__(self, known, unknown_filter):
        datas, targets = self.imgs, self.targets
        new_datas, new_targets = [], []
        for i in range(len(datas)):
            if datas[i][1] in known:
                if unknown_filter:
                    new_item = (datas[i][0], (datas[i][0], known.index(datas[i][1])))
                else:
...
def getTinyImageNetDataset(data_path='./data', unknown_filter=False, dataset_path='tiny-imagenet-200', **args):
    ...
    dataset = Tiny_ImageNet_Filter(os.path.join(data_path, dataset_path, data_split), transform)
    ...
    if 'known_classes' in args:
        ...
        if split == 'in_test' or split == 'train' or split == 'test':
            dataset.__Filter__(known=known_classes, unknown_filter=unknown_filter)
        else:
            dataset.__Filter__(known=unknown_classes, unknown_filter=unknown_filter)
...
class Tiny_ImageNetOpen_Filter(datasets.ImageFolder):
    """Tiny_ImageNet Dataset.
    """
    def __Filter__(self, known):
        ...
        for i in range(len(datas)):
            if datas[i][1] in known:
                ...
                new_datas.append(new_item)
                # new_targets.append(targets[i])
                new_targets.append(len(known))
            if datas[i][1] != 0: print(datas[i][1])

        datas, targets = new_datas, new_targets
        self.samples, self.imgs, self.targets = datas, datas, targets


def getTinyImageNetOpenDataset(data_path='./data', **args):
    mean = (0.4802, 0.4481, 0.3975)
    std = (0.2770, 0.2691, 0.2821)
    ...
    data_split = 'train' if split=='train' else 'val'
    dataset = Tiny_ImageNetOpen_Filter(os.path.join(data_path, 'tiny-imagenet-200', data_split), transform)
...
class COCO_Crop_Filter(datasets.ImageFolder):
    """COCO_Crop Dataset.
    """
    def __Filter__(self, known):
        datas, targets = self.imgs, self.targets
...
def getCOCOCropDataset(data_path='./data', **args):
    mean = (0.4802, 0.4481, 0.3975)
    std = (0.2770, 0.2691, 0.2821)
        
    transform = transforms.Compose([
        LetterBox(args['image_size'], args['image_size']),
        transforms.ToTensor(),
        transforms.Normalize(mean, std),
    ])
    ...
    data_split = 'train' if split=='train' else 'val'
    dataset = COCO_Crop_Filter(os.path.join(data_path, 'coco-crop', data_split), transform)

    if 'known_classes' in args:
        known_classes = sorted(args['known_classes'])
        print(known_classes)
        known_mapping = {val:idx for idx, val in enumerate(known_classes)}
        unknown_classes =  list(set(range(72)) -  set(known_classes))
        unknown_classes = sorted(unknown_classes)
        unknown_mapping = {val: idx+len(known_classes) for idx, val in enumerate(unknown_classes)}
        ...
        dataset.__Filter__(known=[i for i in range(72)])
